# Architecture drift report  (commit eshop-add_forbidden_dep-02)

## New code facts not represented in model
- new edge `csharp:csproj:src/EventBusRabbitMQ/EventBusRabbitMQ.csproj` → `csharp:csproj:src/Ordering.Domain/Ordering.Domain.csproj`
...
- ❌ `csharp:csproj:src/EventBusRabbitMQ/EventBusRabbitMQ.csproj` -/-> `csharp:csproj:src/Ordering.Domain/Ordering.Domain.csproj`: **VIOLATION** — `csharp:csproj:src/EventBusRabbitMQ/EventBusRabbitMQ.csproj`→`csharp:csproj:src/Ordering.Domain/Ordering.Domain.csproj`
