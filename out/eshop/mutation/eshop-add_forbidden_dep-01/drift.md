# Architecture drift report  (commit eshop-add_forbidden_dep-01)

## New code facts not represented in model
- new edge `csharp:csproj:src/Basket.API/Basket.API.csproj` → `csharp:csproj:src/Catalog.API/Catalog.API.csproj`
...
- ❌ `csharp:csproj:src/Basket.API/Basket.API.csproj` -/-> `csharp:csproj:src/Catalog.API/Catalog.API.csproj`: **VIOLATION** — `csharp:csproj:src/Basket.API/Basket.API.csproj`→`csharp:csproj:src/Catalog.API/Catalog.API.csproj`
