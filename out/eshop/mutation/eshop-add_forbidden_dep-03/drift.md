# Architecture drift report  (commit eshop-add_forbidden_dep-03)

## New code facts not represented in model
- new edge `csharp:csproj:src/Ordering.API/Ordering.API.csproj` → `csharp:csproj:src/Catalog.API/Catalog.API.csproj`
...
- ❌ `csharp:csproj:src/Ordering.API/Ordering.API.csproj` -/-> `csharp:csproj:src/Catalog.API/Catalog.API.csproj`: **VIOLATION** — `csharp:csproj:src/Ordering.API/Ordering.API.csproj`→`csharp:csproj:src/Catalog.API/Catalog.API.csproj`
