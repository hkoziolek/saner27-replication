# Architecture drift report  (commit nopcommerce-merge_targets-04)

> **ADVISORY — coverage dropped below baseline** (L0 first-party coverage 97%; floor 80%, plan §11.6). Findings are advisory-only and non-gating for this run.

## New code facts not represented in model
- new edge `csharp:csproj:src/Plugins/Nop.Plugin.ExchangeRate.EcbExchange/Nop.Plugin.ExchangeRate.EcbExchange.csproj` → `csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj`

## Model elements no longer found in code
- _none_

## Coverage gaps (NOT drift — producing layer not extracted in both runs, §11.1)
- edge `csharp:csproj:src/Plugins/Nop.Plugin.ExchangeRate.EcbExchange/Nop.Plugin.ExchangeRate.EcbExchange.csproj` → `csharp:csproj:src/Presentation/Nop.Web.Framework/Nop.Web.Framework.csproj` (coverage-gap)
- edge `csharp:csproj:src/Presentation/Nop.Web.Framework/Nop.Web.Framework.csproj` → `csharp:csproj:src/Libraries/Nop.Core/Nop.Core.csproj` (coverage-gap)
- edge `csharp:csproj:src/Presentation/Nop.Web.Framework/Nop.Web.Framework.csproj` → `csharp:csproj:src/Libraries/Nop.Data/Nop.Data.csproj` (coverage-gap)
- edge `csharp:csproj:src/Presentation/Nop.Web.Framework/Nop.Web.Framework.csproj` → `csharp:csproj:src/Libraries/Nop.Services/Nop.Services.csproj` (coverage-gap)
- edge `csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj` → `csharp:csproj:src/Presentation/Nop.Web.Framework/Nop.Web.Framework.csproj` (coverage-gap)
...
- ❌ `Shipping & Tax Plugins` -/-> `Web Application`: **VIOLATION** — `csharp:csproj:src/Plugins/Nop.Plugin.ExchangeRate.EcbExchange/Nop.Plugin.ExchangeRate.EcbExchange.csproj`→`csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj`, `csharp:csproj:src/Plugins/Nop.Plugin.Shipping.FixedByWeightByTotal/Nop.Plugin.Shipping.FixedByWeightByTotal.csproj`→`csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj`, `csharp:csproj:src/Plugins/Nop.Plugin.Tax.Avalara/Nop.Plugin.Tax.Avalara.csproj`→`csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj`, `csharp:csproj:src/Plugins/Nop.Plugin.Tax.FixedOrByCountryStateZip/Nop.Plugin.Tax.FixedOrByCountryStateZip.csproj`→`csharp:csproj:src/Presentation/Nop.Web/Nop.Web.csproj`
