# Architecture drift report  (commit eshop-rename_target-04)

## New code facts not represented in model
- new target `csharp:csproj:src/WebAppComponents/WebAppComponentsAnonMut.csproj`
- new edge `csharp:csproj:src/HybridApp/HybridApp.csproj` → `csharp:csproj:src/WebAppComponents/WebAppComponentsAnonMut.csproj`
- new edge `csharp:csproj:src/WebApp/WebApp.csproj` → `csharp:csproj:src/WebAppComponents/WebAppComponentsAnonMut.csproj`
...
- removed edge `csharp:csproj:src/WebApp/WebApp.csproj` → `csharp:csproj:src/WebAppComponents/WebAppComponents.csproj`

## Suspected renames / moves (§7.3 — pin via id_aliases)
- `csharp:csproj:src/WebAppComponents/WebAppComponents.csproj` → `csharp:csproj:src/WebAppComponents/WebAppComponentsAnonMut.csproj` (suspected rename/move)
